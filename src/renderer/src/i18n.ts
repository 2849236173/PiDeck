...
const zhCN = {
  ...
  "config.fetchModelsHintOpenai": "获取模型列表失败。OpenAI / Responses API 可能限制了 {baseUrl}/models 端点直接访问。系统已自动补齐 /v1 路径，实际请求为 {baseUrl}/models。建议在「额外模型」中手动添加已知的模型 ID（如 gpt-4o、gpt-4o-mini），然后启动会话后使用 /model 命令确认可用模型。",
  ...
  "config.fetchModelsHintOpenaiCompletions": "获取模型列表失败。OpenAI Chat Completions API 的 {baseUrl}/models 端点可能被限制。如果使用代理或第三方服务，请确认该服务是否支持模型列表查询。不支持时可直接在「额外模型」中手动添加已知模型 ID。",
...
const enUS: Record<TranslationKey, string> = {
  ...
  "config.fetchModelsHintOpenai": "Failed to fetch models. OpenAI / Responses API may restrict direct access to the {baseUrl}/models endpoint. Consider manually adding known model IDs (e.g. gpt-4o, gpt-4o-mini) in 'Extra Models', then use /model command after starting the session to confirm available models.",
  ...
  "config.fetchModelsHintOpenaiCompletions": "Failed to fetch models. The {baseUrl}/models endpoint of OpenAI Chat Completions API may be restricted. If using a proxy or third-party service, verify it supports model listing. If not supported, manually add known model IDs via 'Extra Models'.",
